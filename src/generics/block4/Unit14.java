...
public static class Sendable<T> {
    ...
    Sendable(String from,String to,T content){
...
public static class MailService<T> implements Consumer<Sendable<T>> {
    ...
    public void accept(Sendable<T> tSendable) {
        if(!mailBox.containsKey(tSendable.getTo()))
            mailBox.put(tSendable.getTo(),new ArrayList<T>());
        mailBox.get(tSendable.getTo()).add(tSendable.getContent());

    }
}

public static class MailBox<K,V> extends HashMap<K,List<V>>{
    private Map<String, List<V>> mailBox;
    @Override
